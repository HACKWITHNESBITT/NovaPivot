  ...
  const generateRecommendations = () => {
    if (missingSkills.length === 0) {
      return {
        ...
        description: "You have all the key skills for your target role. Focus on gaining experience and building portfolio projects.",
        priority: "High",
        actions: [
    ...
    if (missingSkills.length <= 2) {
      return {
        title: "Almost There!",
        description: `You're missing just ${missingSkills.length} key skills. With focused learning, you'll be ready in 2-3 months.`,
        priority: "Medium",
        actions: [
          `Focus on: ${missingSkills.join(', ')}`,
    ...
    return {
      ...
      description: `You need to develop ${missingSkills.length} key skills. This is achievable with dedicated effort over 3-6 months.`,
      priority: "High",
      actions: [
        `Priority skills: ${missingSkills.slice(0, 3).join(', ')}`,
        "Create structured learning plan",
        "Consider bootcamp or online courses",
        "Build portfolio projects for each missing skill",
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass rounded-xl p-4 sm:p-6">
          ...
          <div>
            ...
            <div className={`${getScoreBg(matchScore)} border ${getScoreColor(matchScore).replace('text', 'border')} rounded-lg p-6 text-center`}>
              ...
              <div className="mt-4 w-full bg-nova-dark rounded-full h-3">
                <div
                  className={`h-3 rounded-full transition-all ${matchScore >= 70 ? 'bg-green-400' : matchScore >= 40 ? 'bg-yellow-400' : 'bg-red-400'
                    }`}
        ...
        <div className="mt-6">
          <div className={`${getScoreBg(matchScore)} border ${getScoreColor(matchScore).replace('text', 'border')} rounded-lg p-6`}>
            <div className="flex items-center gap-3 mb-4">
              <h3 className={`text-lg font-semibold ${getScoreColor(matchScore)}`}>
                {recommendations.title}
              </h3>
              <span className={`px-3 py-1 text-xs rounded-full ${recommendations.priority === 'High' ? 'bg-red-500/20 text-red-400' :
            ...
            <p className="text-nova-text mb-4">{recommendations.description}</p>

            <div className="space-y-3">
              <h4 className="text-white font-medium mb-2">Recommended Actions:</h4>
              <ul className="space-y-2">
                {recommendations.actions.map((action, index) => (
                  <li key={index} className="flex items-start gap-2 text-nova-text text-sm">
                    <span className="text-nova-teal mt-0.5">•</span>
                    <span>{action}</span>
                  </li>
                ))}
              </ul>
...
// Main App Component (Original - No Auth)
function App() {
